...
export const torneioModel = {
  ...
  listarPorUsuario(usuarioId: number): Torneio[] {
    const rows = db.prepare('SELECT * FROM torneios WHERE usuario_id = ?').all(usuarioId);
    return (rows as any[]).map(paraApi) as Torneio[];
  },
  ...
  inserir(dados: { nome: string; descricao?: string; dataInicio: string; status?: string; camposObrigatorios?: string; usuarioId: number }): Torneio {
    const r = db.prepare(`
      INSERT INTO torneios (nome, descricao, data_inicio, status, campos_obrigatorios, usuario_id, criado_em)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      dados.nome,
      dados.descricao || '',
  ...
  atualizar(id: number, dados: Partial<Omit<Torneio, 'id'>>): Torneio | null {
    ...
    db.prepare(`
      UPDATE torneios
      SET nome = ?, descricao = ?, data_inicio = ?, status = ?, campos_obrigatorios = ?
      WHERE id = ?
    `).run(
      novo.nome,
      novo.descricao,
