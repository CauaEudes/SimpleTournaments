...
const viewLista = document.querySelector('#view-lista');
const viewDetalhe = document.querySelector('#view-detalhe');
const btnVoltar = document.querySelector('#btn-voltar');
const inputBuscaTorneio = document.querySelector('#busca-torneio');
const inputBuscaParticipante = document.querySelector('#busca-participante');
let torneioAtualId = null;
let todosTorneios = [];
let todosParticipantesGlobais = [];
let participantesDoTorneio = [];
let contagemParticipantes = new Map();
...
async function navegarParaDetalhe(id) {
    torneioAtualId = id;
...
async function atualizarTorneios() {
    try {
        ...
        todosParticipantesGlobais.forEach((p) => {
...
async function criarTorneio(dados) {
    limparErro();
    try {
...
async function criarParticipante(dados) {
    limparErro();
    if (!torneioAtualId)
        return;
    try {
